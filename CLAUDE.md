...
uv run registry-mcp-config-check        # missing, mistyped, or do-nothing settings (names only)
...
├── config_report.py       # config_status tool + registry-mcp-config-check: missing/mistyped/do-nothing settings, names only
...
- **Every setting a feature reads is listed in `config_report._FEATURES`**: the settings it can't work without under `needs`, and the rest under `uses`. The `config_status` tool and `registry-mcp-config-check` report a feature that's on but missing a setting it needs, an environment key that's a near-miss of a real setting name, and a setting that does nothing where it is: set to its default, set only for features that are off, or set to an empty value (a problem when that empty value replaces a real default). Always by name, never by value. A new feature adds an entry there, and removing a feature adds its setting prefix to `_RETIRED_PREFIXES`, so leftover keys are reported rather than silently ignored.
